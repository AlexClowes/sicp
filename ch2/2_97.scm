...
(define (install-polynomial-package)
  ...
  (define (pseudoremainder-terms a b)
    (let ((c (coeff (first-term b)))
      ...
      (remainder-terms
        (mul-term-by-all-terms (make-term 0 (expt c (+ 1 (- o1 o2))))
                               a)
        b)))
  ...
  (define (reduce-terms n d)
    (let* ((gcdterms (gcd-terms n d))
           (c (coeff (first-term gcdterms)))
           (o1 (order (first-term n)))
           (o2 (order (first-term d)))
           (integerizing-factor (make-term 0 (expt c (+ 1 (- o1 o2)))))
           (n-reduced (car (div-terms (mul-term-by-all-terms integerizing-factor n)
                                      gcdterms)))
           (d-reduced (car (div-terms (mul-term-by-all-terms integerizing-factor d)
                                      gcdterms)))
           (coeffs-gcd (adjoin-term (make-term 0
                                               (apply gcd
                                                      (append (map coeff n-reduced)
                                                              (map coeff d-reduced))))
                                    (the-empty-termlist))))
      (map (lambda (term-list) (car (div-terms term-list coeffs-gcd)))
